...
def train_vqvae(args, tr_dataloader, va_dataloader, model, optim, scheduler, device):
    ...
    os.makedirs(
        os.path.join(
            args.checkpoint_dir, runs
            ),
        exist_ok=True
        )
    os.makedirs(
        os.path.join(
            args.sample_dir, runs
            ),
        exist_ok=True
        )
    pbar = tqdm(range(args.iter+1), initial=args.start_iter, dynamic_ncols=True, smoothing=0.01)
    ...
    for _epoch in pbar:
        ...
        model.train()
        id_list = []
        for idx, (x, label) in enumerate(tr_dataloader):
            model.zero_grad()

            x = x.to(device)
            x = (x - mean) / std
            recnst, latent_loss, _id = model(x, bypass=_epoch < 0 and args.bypass)
            assert n_latent == _id.shape[-1]
        ...
        if epoch % 10 == 0:
            ...
            torchvision.utils.save_image(
                ...
                os.path.join(args.sample_dir, runs, str(epoch).zfill(5)+".png"),
                nrow=sample_size,
                normalize=True,
                range=(-1, 1),
            )
        ...
        if epoch % args.save_model_iter == 0:
            torch.save(
                    ...
                    f"{args.checkpoint_dir}/{runs}/{str(epoch).zfill(6)}.pt",
            )
...
def train_transformer(args, tr_dataloader, va_dataloader, model, transformer, optim, scheduler, device):
    ...
    #============
    os.makedirs(
        os.path.join(args.transformer_checkpoint_dir, runs + '_genre' if args.w_genres else ''),
        exist_ok=True
    )
    os.makedirs(
        os.path.join(args.transformer_sample_dir, runs + '_genre' if args.w_genres else ''),
        exist_ok=True
    )
    pbar = tqdm(range(args.iter+1), initial=args.start_iter, dynamic_ncols=True, smoothing=0.01)
    ...
    bfn = best_fname(os.path.join(args.checkpoint_dir, runs))
    ckpt_path = os.path.join(args.checkpoint_dir, runs, bfn)
    ...
    for _epoch in pbar:
        ...
        for idx, (x, label) in enumerate(tr_dataloader):
            ...
            p = transformer(_id, label if args.w_genres else None) # [bs, T', n_embed]
        ...
        if epoch % 10 == 0:
            ...
            sample_size = 8
            with torch.no_grad():
                gen, _ = transformer.evaluate(sample_size, n_latent, model.quantize, device, 0 if not args.w_genres else None) #[n_sample, L, embed_dim]
                gen = model.decoder(gen.permute(0,2,1))
            torchvision.utils.save_image(
                gen.unsqueeze(1),
                os.path.join(args.transformer_sample_dir, runs, str(epoch).zfill(5)+".png"),
                nrow=sample_size//2,
                normalize=True,
                range=(-1, 1),
            )
        ...
        if epoch % args.save_model_iter == 0:
            torch.save(
                    ...
                    f"{args.transformer_checkpoint_dir}/{runs + '_genre' if args.w_genres else ''}/{str(epoch).zfill(6)}.pt",
            )
def train_Gentransformer(args, tr_dataloader, va_dataloader, model, transformer, optim, scheduler, device):
    # Options
    print('training transformer with genres condition...')
    n_latent = int(np.array(args.encoder_scale_factors).prod() * 256)
    runs = str(args.embed_dim)+'_'+str(args.n_embed)+'_'+ str(args.latent_loss_weight).split('.')[-1] + '_' + str(n_latent)
    #============
    os.makedirs(
        os.path.join(args.transformer_checkpoint_dir, runs),
        exist_ok=True
    )
    os.makedirs(
        os.path.join(args.transformer_sample_dir, runs),
        exist_ok=True
    )
    pbar = tqdm(range(args.iter+1), initial=args.start_iter, dynamic_ncols=True, smoothing=0.01)
    ...
    for _epoch in pbar:
        ...
        if epoch % 10 == 0:
            ...
            sample_size = 8
            with torch.no_grad():
                gen, _ = transformer.evaluate(sample_size, n_latent, model.quantize, device) #[n_sample, L, embed_dim]
                gen = model.decoder(gen.permute(0,2,1))
            torchvision.utils.save_image(
                gen.unsqueeze(1),
                os.path.join(args.transformer_sample_dir, runs, str(epoch).zfill(5)+".png"),
        ...
        if epoch % args.save_model_iter == 0:
            torch.save(
                    ...
                    f"{args.transformer_checkpoint_dir}/{runs}/{str(epoch).zfill(6)}.pt",
            )
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--dataset', type=str, choices=['freesound', 'looperman'], help='dataset to use'
    )
    ...
    parser.add_argument(
        '--w_genres', action='store_true', help='train transformer with paired genres data', 
    )

    args = parser.parse_args()
    if args.dataset == 'freesound' and args.w_genres == True:
        raise RuntimeError('Freesound does not support training with genres condition')
    ...
    tr_dataloader, num_tr, va_dataloader, num_va = data.get_genre_datasets(args.data_path, "", args.batch, args.valid_sample_ratio)
    ...
    if wandb is not None and args.wandb:
        wandb.init(
            project=f"LoopGeneration-{args.mode}", 
            entity="yklego", 
            config=args, 
            name= str(args.dataset) + ' ' + str(args.embed_dim)+' '+str(args.n_embed)+' '+ str(args.latent_loss_weight)+ ' ' + str(n_latent) + ' genre' if args.w_genres else '')
        print('using wandb')
