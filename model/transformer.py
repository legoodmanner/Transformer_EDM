...
class VQTransformer(nn.Module):
    def __init__(self, d_model, n_layer, n_embed, softmax_temp=None):
        ...
        self.token_embed = nn.Embedding(n_embed+66, d_model)
        self.output_mapping = nn.Linear(d_model, n_embed)

    
    def forward(self, x, label=None, attn_mask=None, length_mask=None):
        ...
        if label is None:
            ...
            # start token id: n_embed ~ n_embed + n_genre
            start = (label + self.n_embed).unsqueeze(1)
    ...
    def evaluate(self, n_sample, L, quantizer, device, genres_id=None, threshold_p=1):
        if genres_id is None:
            x = torch.randint(self.n_embed, self.n_embed+66, (n_sample, 1)).long().to(device) #[n_sample, 1]
        else:
            x = torch.LongTensor([[self.n_embed + genres_id]] * n_sample).to(device) #[n_sample, 1]
        genre_ids = x.clone()
        pl = []
        for l in range(0, L):
        ...
        return pred_quant, pl, genre_ids.squeeze() #[n_sample, L, embed_dim]
